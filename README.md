...
Calls db.init_db() to create the margins table if it doesn't exist.
Calls db.get_all_dates() to load all dates already in the database (currently just 2026-02-27).
Generates all weekdays (Monday–Friday) from 2022-01-01 to today using generate_weekdays().
Computes the list of missing dates = all weekdays minus dates already in DB.
Iterates through each missing date, calling fetch_and_save(date_str) which:
Launches a headless Chromium browser
Visits https://www.mcxccl.com/ (homepage, to bypass Akamai)
Navigates to https://www.mcxccl.com/risk-management/daily-margin
Fills the date fields and clicks "Show"
...
Filters for NATURALGAS and NATGASMINI only
Upserts matching records into data/margins.db
Waits 3 seconds (REQUEST_DELAY = 3) between each date request.
Prints a final summary of dates attempted, dates with data, holidays/empty days, errors, and total records saved.
How long will it take?
...
Checks out the repository (including the current data/margins.db file) using actions/checkout@v4.
Sets up Python 3.10 using actions/setup-python@v5.
Installs dependencies: runs pip install -r requirements.txt (installs playwright, pandas, openpyxl, python-dateutil), then installs the Chromium browser binary via python -m playwright install chromium and its system dependencies via python -m playwright install-deps chromium.
Fetches today's margin data: runs python main.py $(date +%Y-%m-%d) — this scrapes the MCX CCL website for today's date and saves NATURALGAS and NATGASMINI records into data/margins.db.
Commits and pushes the updated database back to the repo:
Configures git with the github-actions[bot] identity
Stages the entire data/ directory (git add data/)
Only commits if there are actual changes (git diff --staged --quiet || git commit -m "data: daily margin YYYY-MM-DD")
Pushes using the built-in GITHUB_TOKEN secret
...
Writes the file exports/mcx_margins.xlsx
The Excel file has two sheets:
"Daily Margins" — all records for NATURALGAS and NATGASMINI, ordered by date DESC, symbol ASC, expiry ASC, with columns: Date, Symbol, Expiry, Instrument ID, File ID, Initial Margin %, ELM %, Tender Margin %, Total Margin %, Additional Long Margin %, Additional Short Margin %, Special Long Margin %, Special Short Margin %, Delivery Margin %, Daily Volatility, Annualized Volatility, Created At
"Summary" — per-symbol statistics: Symbol, Record Count, Earliest Date, Latest Date, Avg Initial Margin %, Min Initial Margin %, Max Initial Margin %
Column widths are auto-fitted (capped at 30 characters wide)
Requires pandas and openpyxl to be installed (both are in requirements.txt)
The export query (in query.py) explicitly filters WHERE symbol IN ('NATURALGAS', 'NATGASMINI'), so only those two symbols appear in the Excel regardless of what else might be in the database.
...
Homepage-first navigation (lines 104–112): visits https://www.mcxccl.com/ first to acquire Akamai session cookies before hitting the data page.
Stealth JS injection (lines 41–47, applied via add_init_script): removes navigator.webdriver, fakes window.chrome, sets realistic languages and hardwareConcurrency.
Realistic browser context (lines 73–83): Windows Chrome 120 user-agent, 1920×1080 viewport, en-US locale, Asia/Kolkata timezone, full sec-ch-ua headers.
Launch args (lines 22–29): --disable-blink-features=AutomationControlled suppresses the automation flag.
API response interception (lines 89–101): intercepts the POST /backpage.aspx/GetDailyMargin JSON response directly — no DOM scraping.
Correct dual-format date handling: fills #txtDate as DD/MM/YYYY (display) and cph_InnerContainerRight_C001_txtDate_hid_val as YYYYMMDD (what the API reads).
Q2: Filtering in main.py — ✅ Only NATURALGAS and NATGASMINI
Exact filter logic (main.py lines 14, 57–59):
